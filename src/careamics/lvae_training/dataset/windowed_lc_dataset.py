...
class WindowedLCDLoader(MultiChDloader):
    ...
    def total_patch_count(self) -> int:
        ...
        spatial_dims = self._data.shape[1:-1]
        patch_spatial_dims = (64,64)
        stride_spatial = (4,4)
        n_patches_per_dim = []
        for i in range(len(spatial_dims)):
            padded_dim = self._data.shape[i]
            patch_dim = patch_spatial_dims[i]
            stride_dim = stride_spatial[i]
            # Standard formula for strided convolutions/windows.
            num_patches = np.floor((padded_dim - patch_dim) / stride_dim).astype(int) + 1
        n_patches_per_dim.append(num_patches)

        # Non-spatial dims (N, C) are not tiled, their count is their size.
        grid_counts_for_flat_idx = [self._data.shape[0]] + n_patches_per_dim
        return int(np.prod(grid_counts_for_flat_idx))
