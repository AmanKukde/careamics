...
class WindowedTilingDloader(MultiChDloader):
    ...
    def set_img_sz(self, image_size, grid_size: Union[int, Tuple[int, int, int]]):
        ...
        self.pad_amount_one_edge, self.padded_data_shape = self.get_padding_dimensions_and_shape(stride = stride_spatial)
    ...
    def get_padding_dimensions_and_shape(self, stride=None):
        """
        Compute per-edge padding and padded data shape for 3D data,
        using explicit stride if provided. Pads only as necessary to fit tiles.

        Args:
            stride: list or tuple of ints [Z, H, W] to explicitly set step size.
                    If None, it will be computed from grid_sz // sqrt(mmse_repetitions)

        Returns:
            pad_amount_one_edge: list of ints, padding on one side per axis [Z,H,W]
        ...
        if stride is not None:
            ...
            step_size = [max(1, g // int(math.sqrt(64))) for g in grid_sz]

        # --- Compute number of tiles per axis (ceil division) ---
        n_tiles = [
            math.ceil((self._data.shape[i+1] - grid_sz[i]) / step_size[i]) + 1
        ...
        total_padding = [
            needed_length[i] - self._data.shape[i+1]
            for i in range(spatial_dims)
        ]
        ...
        padded_shape_list = list(self._data.shape)
        for i in range(spatial_dims):
            padded_shape_list[i+1] += total_padding[i]
        padded_data_shape = tuple(padded_shape_list)
